	...
	var $num = 0;
	 ...
	 function create() {
		...
		if (!empty($this->lists)) {
			...
			$this->moduleclass_sfx = 'box_sendnews';
			...
			if($this->dropdown){
			 ...
			 if (!$loggedin) {
				if ($GLOBALS[ACA.'allow_unregistered']) {
					 ...
					 if (!$this->shownamefield) {
						$text = '<label>Navn: </label><input id="wz_11" type="text" size="'. $this->fieldsize.'" value="" class="box_sendnews_txt" name="name" /><br class="" />';
						$h .= acajoom::printLine($this->linear, $text);
					 } else {
						$text = '<input id="wz_11" type="hidden" value="" name="name" />';
					 }

					$text = '<label>E-mail: </label><input id="wz_12" type="text" size="' .$this->fieldsize .'" value="" class="inputbox box_sendnews_txt" name="email" /><br class="" />';
					$h .= acajoom::printLine($this->linear, $text);
			...
			$this->imgRegistered = 'templates/defrieisenkram/img/tilmeld-btn.png';
			$this->imgUnregistered = 'templates/defrieisenkram/img/tilmeld-btn.png';
			$this->redirectURL = 'index.php?option=com_kontakt';
			...
			if (!$loggedin) {
				...
				if ($GLOBALS[ACA.'allow_unregistered']) {
					...
					$text = '<label>&nbsp;</label><input id="aca_22" type="image" src="'.$this->imgUnregistered.'" value="'.$this->buttonUnregistered.'" alt="'.$this->buttonUnregistered.'" name="'.$this->buttonUnregistered.'" onclick="return submitacajoommod'.$this->num.'(\'modacajoomForm'.$this->num.'\');" />';
					else
					$text = '<label>&nbsp;</label><input id="aca_22" type="button" value="'.$this->buttonUnregistered.'" class="button" name="'.$this->buttonUnregistered.'" onclick="return submitacajoommod'.$this->num.'(\'modacajoomForm'.$this->num.'\');" />';
					$h .= acajoom::printLine($this->linear, $text);
					$h .= '
					</div>
