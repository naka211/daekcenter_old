...
/**
* Dfi_order_product Table class
*
* @package		Joomla
* @subpackage	Dfi_order_product
* @since 1.0
*/
class TableDfi_order_product extends JTable
{
	...
	function __construct(& $db) {
		parent::__construct('#__dfi_order_products', 'dfi_order_product_id', $db);
	}
	...
	*/
	...
	function check()
	{
		if (!$this->dfi_order_product_id)
		{
			// check insert
			$query = 'SELECT a.dfi_order_product_id FROM #__dfi_order_products a' .
				' WHERE a.dfi_product_id = '.$this->dfi_product_id.' AND a.dfi_order_id = '.$this->dfi_order_id;
			$this->_db->setQuery($query);
			$xid = intval($this->_db->loadResult());
			if ($xid)
			{
				$this->dfi_order_product_id = $xid;
				return false;
			}
			return true;
		} else {
			// check update
			$query = 'SELECT a.dfi_order_product_id FROM #__dfi_order_products a' .
				' WHERE a.dfi_product_id = '.$this->dfi_product_id.' AND a.dfi_order_id = '.$this->dfi_order_id.' AND dfi_order_product_id != '.$this->dfi_order_product_id;
			$this->_db->setQuery($query);
			$xid = intval($this->_db->loadResult());
			if ($xid)
			{
