...
class Dfi_orderHelper
{
	function dropdown($name, $value, $options=NULL, $autosubmit=TRUE, $size=1, $extra='')
	{
		...
		Dfi_orderHelper::full_paths(0, $items);
	...
	function full_paths($parent, &$items)
	{
		...
		$query = 'SELECT a.dfi_order_id value, a.dfi_order_id text' .
				' FROM #__dfi_orders a' .
				//' WHERE a.parent=' .$parent.
				' ORDER BY a.dfi_order_id';
		$db->setQuery($query);
		...
		if ($res)
		{
			foreach ($res as $o)
			{
				//$o->text = Dfi_orderHelper::full_path($o->value);
				$items[] = $o;
				//Dfi_orderHelper::full_paths($o->value, $items);
			}
		}
	}
	...
	/*function full_path($id)
	{
		...
		$name = $r->dfi_order_id;
		if ($r->parent)
			return Dfi_orderHelper::full_path($r->parent).' > '.$name;
		else
			return $name;
	}*/
