 ...
 */
class Dfi_kobreak_productHelper
{
	function dropdown($name, $value, $options=NULL, $autosubmit=TRUE, $size=1, $extra='')
	{
		...
		Dfi_kobreak_productHelper::full_paths(0, $items);
	...
	function full_paths($parent, &$items)
	{
		...
		$query = 'SELECT a.dfi_kobreak_product_id value, a.dfi_kobreak_product_id text' .
				' FROM #__dfi_kobreak_products a' .
				//' WHERE a.parent=' .$parent.
				' ORDER BY a.dfi_kobreak_product_id';
		$db->setQuery($query);
		...
		if ($res)
		{
			foreach ($res as $o)
			{
				//$o->text = Dfi_kobreak_productHelper::full_path($o->value);
				$items[] = $o;
				//Dfi_kobreak_productHelper::full_paths($o->value, $items);
			}
		}
	}
	...
	/*function full_path($id)
	{
		...
		$query = 'SELECT a.*' .
				' FROM #__dfi_kobreak_products a' .
				' WHERE a.dfi_kobreak_product_id='.$id;
		$db->setQuery($query);				
		$r = $db->loadObject();
		
		if (!$r) return '?';		
		$name = $r->dfi_kobreak_product_id;
		if ($r->parent)
			return Dfi_kobreak_productHelper::full_path($r->parent).' > '.$name;
		else
			return $name;
	}*/
