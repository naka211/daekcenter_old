...
/**
* Dfi_member Table class
*
* @package		Joomla
* @subpackage	Dfi_member
* @since 1.0
*/
class TableDfi_member extends JTable
{
	var $dfi_member_id;
	var $user_id;
	var $dfi_shop_id;
	...
	function __construct(& $db) {
		parent::__construct('#__dfi_members', 'dfi_member_id', $db);
	}
	...
	*/
	...
	function check()
	{
		if (!$this->dfi_member_id)
		{
			// check insert
			$query = 'SELECT a.dfi_member_id FROM #__dfi_members a' .
				' WHERE a.user_id = '.$this->user_id.' AND a.dfi_shop_id='.$this->dfi_shop_id;
			$this->_db->setQuery($query);
			$xid = intval($this->_db->loadResult());
			if ($xid)
			{
				$this->dfi_member_id = $xid;
			}
			return !$xid;
		} else {
			// check update
