 ...
 * @since 1.0
 */
class Dfi_productsViewDfi_add_kampagne extends JView
{
	...
	function _displayForm($tpl)
	{
		...
		if($wee > 0){
            ...
            $v_hvidpris = ((((float)$dfi_product->hvidpris * 0.8 ) - ((float)$dfi_product->nettopris + $wee )) / ((float)$dfi_product->hvidpris * 0.8 ))*100;
			$v_hvidpris=round($v_hvidpris);
          
			$v_rodpris = ((((float)$dfi_product->rodpris * 0.8 ) - ((float)$dfi_product->nettopris + $wee )) / ((float)$dfi_product->rodpris * 0.8 ))*100;
			$v_rodpris=round($v_rodpris);
			
			$v_nuspris = ((((float)$dfi_product->nupris * 0.8 ) - ((float)$dfi_product->nettopris + $wee )) / ((float)$dfi_product->nupris * 0.8 ))*100;
