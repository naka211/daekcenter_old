...
/**
* Dfi_report Table class
*
* @package		Joomla
* @subpackage	Dfi_report
* @since 1.0
*/
class TableDfi_report extends JTable
{
	var $dfi_report_id;
	var $dfi_shop_id;
	var $dfi_product_id;
	...
	function __construct(& $db) {
		parent::__construct('#__dfi_reports', 'dfi_report_id', $db);
	}
	...
	*/
	...
	function check()
	{
		if (!$this->dfi_report_id)
		{
			// check insert
			//$query = 'SELECT a.dfi_report_id FROM #__dfi_reports a' .
			//	' WHERE a.dfi_report_id = '.$this->dfi_report_id;
			//$this->_db->setQuery($query);
			//$xid = intval($this->_db->loadResult());
			//if ($xid)
			//{
			//	$this->dfi_report_id = $xid;
			//}
			//return !$xid;
		} else {
			// check update
