...
class Dfi_folderHelper
{
	function dropdown($name, $value, $options=NULL, $autosubmit=TRUE, $size=1, $extra='')
	{
		...
		Dfi_folderHelper::full_paths(0, $items);
	...
	function full_paths($parent, &$items)
	{
		...
		$query = 'SELECT a.dfi_order_product_folder_id value, a.dfi_order_product_folder_id text' .
				' FROM #__dfi_order_product_folders a' .
				//' WHERE a.parent=' .$parent.
				' ORDER BY a.dfi_order_product_folder_id';
		$db->setQuery($query);
		...
		if ($res)
		{
			foreach ($res as $o)
			{
				//$o->text = Dfi_folderHelper::full_path($o->value);
				$items[] = $o;
				//Dfi_folderHelper::full_paths($o->value, $items);
			}
		}
	}
	...
	/*function full_path($id)
	{
		...
		$query = 'SELECT a.*' .
				' FROM #__dfi_order_product_folders a' .
				' WHERE a.dfi_order_product_folder_id='.$id;
		$db->setQuery($query);				
		$r = $db->loadObject();
		
		if (!$r) return '?';		
		$name = $r->dfi_order_product_folder_id;
		if ($r->parent)
			return Dfi_folderHelper::full_path($r->parent).' > '.$name;
		else
			return $name;
	}*/
