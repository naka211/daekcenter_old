...
class RokQuickCartControllerFile extends RokQuickCartController
{
	...
	function upload()
	{
		...
		if (isset($file['name'])) {
			$filepath = JPath::clean(COM_ROKQUICKCART_BASE.DS.$folder.DS.strtolower($file['name']));
	...
	function delete()
	{
		...
		if (count($paths)) {
			foreach ($paths as $path)
			{
				...
				$fullPath = JPath::clean(COM_ROKQUICKCART_BASE.DS.$folder.DS.$path);
				if (is_file($fullPath)) {
		...
		if ($tmpl == 'component') {
			...
			$mainframe->redirect('index.php?option=com_rokquickcart&view=mediaList&folder='.$folder.'&tmpl=component');
		} else {
			$mainframe->redirect('index.php?option=com_rokquickcart&folder='.$folder);
		}
	}
}
