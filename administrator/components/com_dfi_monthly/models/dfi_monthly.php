 ...
 * @since 1.5
 */
class Dfi_monthlyModelDfi_monthly extends JModel
{
	/**
	 * Category ata array
	 *
	...
	function _buildQuery()
	{
	  ...
	  if($filter_year >  0){
		...
		$query = ' SELECT a.*,s.*'//,cc.title AS category'
			. ' FROM #__dfi_shops as s  '
			. ' LEFT JOIN  #__dfi_client_pris AS a ON s.dfi_shop_id = a.shop_id '
			. ' AND a.year = '.$year
	...
	function _buildContentWhere()
	{
		...
		if ($search) {			
			$_where[] = 'LOWER(s.company_name) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
