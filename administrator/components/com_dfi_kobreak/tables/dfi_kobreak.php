...
/**
* Dfi_kobreak Table class
*
* @package		Joomla
* @subpackage	Dfi_kobreak
* @since 1.0
*/
class TableDfi_kobreak extends JTable
{
	var $dfi_kobreak_id;
	var $dfi_supplier_id;
	var $lev_uge;
	...
	function __construct(& $db) {
		parent::__construct('#__dfi_kobreaks', 'dfi_kobreak_id', $db);
	}
	...
	*/
	...
	function check()
	{
		if (!$this->dfi_kobreak_id)
		{
			// check insert
			//$query = 'SELECT a.dfi_kobreak_id FROM #__dfi_kobreaks a' .
			//	' WHERE a.dfi_kobreak_id = '.$this->dfi_kobreak_id;
			//$this->_db->setQuery($query);
			//$xid = intval($this->_db->loadResult());
			//if ($xid)
			//{
			//	$this->dfi_kobreak_id = $xid;
			//}
			//return !$xid;
		} else {
			// check update
