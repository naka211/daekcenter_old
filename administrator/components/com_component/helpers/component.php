...
class ComponentHelper
{
	function dropdown($name, $value, $options=NULL, $autosubmit=TRUE, $size=1, $extra='')
	{
		...
		ComponentHelper::full_paths(0, $items);
	...
	function full_paths($parent, &$items)
	{
		...
		$query = 'SELECT a.id value, a.name text' .
				' FROM #__components a' .
		...
		if ($res)
		{
			foreach ($res as $o)
			{
				$o->text = ComponentHelper::full_path($o->value);
				$items[] = $o;
				ComponentHelper::full_paths($o->value, $items);
			}
		}
	}
	...
	function full_path($id)
	{
		...
		$name = $r->name;
		if ($r->parent)
			return ComponentHelper::full_path($r->parent).' > '.$name;
		else
			return $name;
	}
