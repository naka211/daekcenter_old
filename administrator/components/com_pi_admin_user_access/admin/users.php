...
for($i=0; $i < count( $rows ); $i++) {
	...
	$usergroupName = getUsergroupName($row->pi_usergroup);		
	echo '<tr class="row'.$k.'"><td width="25%">'.$row->username.'</td><td>'.$row->name.'</td><td><a href="mailto:'.$row->email.'">'.$row->email;
	echo '</a></td>';
	
	if($class_ua->ua_config['show_joomla_group']){
		echo '<td>';
		if($class_ua->ua_config['disable_joomla_group_selectbox']){
			/*
			if($row->gid==23){
				echo 'manager';
			}else if($row->gid==24){
		...
		echo '</td>';
	}//end if display_joomlagroup_select
	...
	foreach($usergroups as $usergroup){
		if($usergroup->id==$row->pi_usergroup){
			$selected = ' selected="selected"';
		}else{
			$selected = '';
		}		
