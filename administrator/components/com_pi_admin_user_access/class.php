...
class class_ua{
	...
	var $version = '2.0.3';
	var $module_frontend_installed = false;
	var $module_frontend_show_title = false;
	var $module_frontend_show_on_all_pages = false;
	var $module_published_frontend;
	var $itemtypes = array();	
...
',$component);
			...
			$component_array = array($component[0],$component[1],$component[2],$component[3]);
		...
		if( defined('_JEXEC') ){
			//joomla 1.5
			$mod_dir = 'modules/mod_admin_user_access_frontend';
		}else{
			//joomla 1.0.x
			$mod_dir = 'modules';
		}		
		//check if mod frontend is installed, published, title is hidden and assigned to all
		if(file_exists(dirname(__FILE__).'/../../../'.$mod_dir.'/mod_admin_user_access_frontend.php')){
			$this->module_frontend_installed = true;	
			//check if mudule is published and title is displayed
			$this->db->setQuery("SELECT id, published, showtitle FROM #__modules WHERE module='mod_admin_user_access_frontend' LIMIT 1");
			$rows = $this->db->loadObjectList();
			$row = $rows[0];						
			if($row->published==1){
				$this->module_published_frontend = true;
			}
			if($row->showtitle){
				$this->module_frontend_show_title = true;
			}
			$module_id = $row->id;
			//check if module is assigned to all pages
			$this->db->setQuery("SELECT menuid FROM #__modules_menu WHERE moduleid='$module_id' LIMIT 1");
			$rows = $this->db->loadObjectList();
			$row = $rows[0];
			if($row->menuid==0 && count($rows)){
				$this->module_frontend_show_on_all_pages = true;
			}						
	...
	function echo_header(){	
		...
		echo '<link href="components/com_pi_admin_user_access/css/pi_admin_user_access2.css" rel="stylesheet" type="text/css" />'."\n";
		...
		//message if module frontend is not installed	
		if(!$this->module_frontend_installed){				
			echo '<div style="color: red; text-align: left;">'._pi_ua_lang_warning.' '._pi_ua_lang_modnotinstalled.' (mod_admin_user_access_frontend)<br/><br/></div>';
		}
		
		//message if module frontend is not published	
		if(!$this->module_published_frontend){				
			echo '<div style="color: red; text-align: left;">'.strtolower(_pi_ua_lang_modnotpublished).' (mod_admin_user_access_frontend)<br/><br/></div>';
		}	
		
		//message if module frontend is not on each page
		if(!$this->module_frontend_show_on_all_pages){				
			echo '<div style="color: red; text-align: left;">'._pi_ua_lang_mod_not_show_on_all_pages.' (mod_admin_user_access_frontend)<br/><br/></div>';
		}
	...
	function usergroup_save(){			
		...
		if($id==''){
			//new usergroup
			$this->db->setQuery( "INSERT INTO #__pi_aua_usergroups SET name='$name', email='$email', description='$description' ");
			if (!$this->db->query()) {
			...
			//edit usergroup
			$this->db->setQuery( "UPDATE #__pi_aua_usergroups SET name='$name', email='$email', description='$description' WHERE id='$id' ");
			if (!$this->db->query()) {
				echo "<script> alert('".$this->db->getErrorMsg()."'); window.history.go(-1); </script>";
				exit();
			}
...
redirect_to_pi='.$this->get_var('redirect_to_pi', 'false', 'post').'
...
if(is_array($components)){		
	for($n = 0; $n < count($components); $n++){	
		if(defined('_JEXEC')){
			...
			$component_name = $components[$n]['name'];
			$component_alias = $components[$n]['alias'];
			$component_link = $components[$n]['link'];
			$component_dropdown = $components[$n]['dropdown'];
		}
		if($n!=0){
			$config .= ',';	
		}
		$config .= $this->clean_config_for_db($component_name).';'.$this->clean_config_for_db($component_alias).';'.$this->clean_config_for_db($component_link).';'.$component_dropdown;
	}	
}
